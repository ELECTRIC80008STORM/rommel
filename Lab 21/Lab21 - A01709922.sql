...
/*Laboratorio 21*/
select sum(e.cantidad) as 'Cantidad de Entregas', sum(e.cantidad * (m.precio + m.impuesto)) as 'Importe Total'
from  entregan as e
join materiales as m on e.clave = m.clave
where year(e.fecha) = 1997;

select razonsocial as 'Razon Social',
numero as 'No. De Entregas',
...
select m.clave as 'Clave', m.descripcion as 'Descripción',
...
select p.razonsocial as 'Razon Social', avg(e.cantidad) as 'Cantidad Promedio',
m.clave as 'Clave del Material', m.descripcion as 'Descripción'
from proveedores as p
join entregan as e on p.rfc = e.rfc
join materiales as m on e.clave = m.clave
group by m.clave
...
(1031, 'Varilla 5/34', 150, 15, 2.08),
(1032, 'Varilla 5/35', 160, 16, 2.10),
(1033, 'Varilla 5/36', 170, 17, 2.12),
(1034, 'Varilla 6/37', 180, 18, 2.14),
(1035, 'Varilla 6/38', 190, 19, 2.16);
...
where pr.denominacion = 'Vamos México' or pr.denominacion = 'Querétaro Limpio'
);
...
/*VAGO780901 fue cambiado por GGGG800101 para tener registros*/
...
where p2.rfc = 'GGGG800101'
);

create view entregastotales2000 as
select e.rfc,
sum(e.cantidad) as total2000
from entregan as e
where year(e.fecha) = 2000
group by e.rfc;

create view entregastotales2001 as
select e.rfc,
sum(e.cantidad) as total2001
from entregan as e
where year(e.fecha) = 2001
group by e.rfc;

select p.rfc as 'RFC', p.razonsocial as 'Razon Social'
from proveedores as p
join entregastotales2000 as et2000 on p.rfc = et2000.rfc
join entregastotales2001 as et2001 on p.rfc = et2001.rfc
join entregan as e on p.rfc = e.rfc
join proyectos as pr on e.numero = pr.numero
where pr.denominacion = 'Infonavit Durango' and
et2000.total2000 > et2001.total2001;
